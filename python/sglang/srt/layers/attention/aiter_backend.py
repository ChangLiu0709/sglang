...
class AiterAttnBackend(AttentionBackend):
    ...
    def forward_extend(
        ...
        **kwargs,
        ...
        if self.use_mla:
            ...
            # mha_batch_prefill_func (aiter) requires K/V linear layout with matching
            # middle dims; models with qk_head_dim != v_head_dim (e.g. MiMo-V2-Flash)
            # fail with "K/V linear layout must match page size and head count".
            if layer.qk_head_dim != layer.v_head_dim:
                # mha_batch_prefill_func assumes K/V share the same middle dims; use Triton
                # extend kernel (same as triton attention backend) for GQA + V head dim.
                o = q.new_empty(
                    (q.shape[0], layer.tp_q_head_num * layer.v_head_dim),
                    dtype=q.dtype,
                )
                ...
                if (
                    ...
                    or layer.attn_type == AttentionType.DECODER_BIDIRECTIONAL
                ):
                    causal = False
                sliding_window_size = (
    ...
    def forward_decode(
        ...
        **kwargs,
        ...
        # kwargs: e.g. ``sinks`` (MiMo SWA); not used by Aiter kernels today.
        _ = kwargs
        ...
        if self.use_mla:
            ...
            # aiter paged_attention_ragged uses head_size = query.size(2) (see
            # pa_ragged.py); ``out`` must be [num_seqs, num_heads, qk_head_dim] or the
            # kernel writes OOB (MiMo: qk=192, v=128). Slice to v_head_dim after the call.
            o_pa = q.new_empty(
                (q.shape[0], layer.tp_q_head_num * layer.qk_head_dim),
                dtype=self.input_dtype,
            )
            ...
            paged_attention_ragged(
                o_pa.view(-1, layer.tp_q_head_num, layer.qk_head_dim),
