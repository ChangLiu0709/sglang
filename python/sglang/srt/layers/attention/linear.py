...
class QKVParallelLinear(ColumnParallelLinear):
    ...
    @staticmethod
    def _qkv_block_rows_for_logical_shard(
        loaded_shard_id: str,
        tp_rank: int,
        tp_size: int,
        ...
        """Block row start and count for this rank's Q/K/V shard (FP8 block scales)."""
        if loaded_shard_id == "q":
            elem_per = num_heads * head_size
            logical_id = tp_rank
        else:
            num_logical = tp_size // num_kv_head_replicas
            if loaded_shard_id == "k":
                elem_total = total_num_kv_heads * head_size
            else:
                elem_total = total_num_kv_heads * v_head_size
            elem_per = elem_total // num_logical
            logical_id = tp_rank // num_kv_head_replicas
        begin_elem = logical_id * elem_per
        end_elem = begin_elem + elem_per
        start_row = begin_elem // block_n
        n_rows = (end_elem + block_n - 1) // block_n - start_row
        return start_row, n_rows

    def _qkv_local_block_layout(self, block_n: int) -> tuple:
        """Q/K/V block offsets and sizes in the fused per-rank scale tensor."""
        q_blk = (self.num_heads * self.head_size + block_n - 1) // block_n
        k_blk = (self.num_kv_heads * self.head_size + block_n - 1) // block_n
        v_blk = (self.num_kv_heads * self.v_head_size + block_n - 1) // block_n
        offsets = {"q": 0, "k": q_blk, "v": q_blk + k_blk}
        sizes = {"q": q_blk, "k": k_blk, "v": v_blk}
    ...
    def _load_qkv_block_scale(
        ...
        block_n = 1 if getattr(param, "format_ue8m0", False) else block_n
        q_size = (self.total_num_heads * self.head_size + block_n - 1) // block_n
        k_size = (self.total_num_kv_heads * self.head_size + block_n - 1) // block_n
        v_size = (
            self.total_num_kv_heads * self.v_head_size + block_n - 1
        ) // block_n
        shard_offsets = [
            # (shard_id, shard_offset, shard_size)
            ("q", 0, q_size),
        ...
        local_offsets, local_sizes = self._qkv_local_block_layout(block_n)
        for shard_id, shard_offset, shard_size in shard_offsets:
            loaded_weight_shard = loaded_weight.narrow(
                param.output_dim, shard_offset, shard_size
            )
            rank_shard_offset = local_offsets[shard_id]
            rank_shard_size = local_sizes[shard_id]
            if not self.use_presharded_weights:
                sb, nrows = self._qkv_block_rows_for_logical_shard(
                    shard_id,
                    self.tp_rank,
                    self.tp_size,
    ...
    def weight_loader_v2(
        ...
        if isinstance(param, BlockQuantScaleParameter):
            ...
            block_n = 1 if getattr(param, "format_ue8m0", False) else raw_block_n
            local_offsets, local_sizes = self._qkv_local_block_layout(block_n)
            rank_shard_offset = local_offsets[loaded_shard_id]
            rank_shard_size = local_sizes[loaded_shard_id]
            if not self.use_presharded_weights:
                sb, nrows = self._qkv_block_rows_for_logical_shard(
                    loaded_shard_id,
                    self.tp_rank,
                ...
                loaded_weight = loaded_weight.narrow(param.output_dim, sb, nrows)
            param.load_qkv_weight(
                loaded_weight=loaded_weight,
                num_heads=self.num_kv_head_replicas,
                shard_id=loaded_shard_id,
